...
/**
  ...
  */
class LatConnectorTest extends FlatSpec with Matchers {
  ...
  "A correct query" should "be executed correctly" in {
    ...
    val connector = new LatConnector(coordinate, latitude, degrees, minutes, seconds, direction)
  ...
  "An empty query " should "return an empty ListBuffer" in {
    ...
    val connector = new LatConnector(null,null,null,null,null,null)
    val response = connector.execute()
