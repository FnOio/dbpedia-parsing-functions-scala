...
import dataparsers.DateTimeParserConfig._
import dataparsers.ontology.datatypes.Datatype
import dataparsers.util.{Date, Language, Redirects}
import dataparsers.util.wikiparser.{Node, TemplateNode, TextNode}
                     ...
                     val strict : Boolean = false) extends DataParser
{
    ...
    private val language = if(supportedLanguages.contains(context.language.wikiCode)) context.language.wikiCode else "en"

    private val months = monthsMap.getOrElse(language, monthsMap("en"))
    private val eraStr = eraStrMap.getOrElse(language, eraStrMap("en"))
    private val cardinalityRegex = cardinalityRegexMap.getOrElse(language, cardinalityRegexMap("en"))
    private val templates = templateDateMap.getOrElse(language, Map())
