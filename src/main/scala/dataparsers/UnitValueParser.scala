...
import dataparsers.ontology.Ontology
import dataparsers.ontology.datatypes.{Datatype, DimensionDatatype, UnitDatatype}
import dataparsers.util.wikiparser.{Node, PropertyNode, TemplateNode, TextNode}
import dataparsers.util.{Language, ParserUtils, Redirects}
                        ...
                        multiplicationFactor : Double = 1.0) extends DataParser
{
    ...
    private val parserUtils = new ParserUtils(extractionContext.language)
    ...
    {
        if (language == "en" || language == "ja" || language == "zh" || language == "commons") //wmaroy added commons for getting same results as en
        {
            input match
            {
    ...
    private def generateOutput(valueString : String, unitString : Option[String] = None, errors : Option[ParsingErrors]) : Option[(Double, UnitDatatype)] =
    {
        val value = 
        {
            try
            {
                parserUtils.parse(valueString).doubleValue * multiplicationFactor * 1.0
            }
            catch
            {
