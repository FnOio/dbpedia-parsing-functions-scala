...
import util.text.html.{HtmlCoder, XmlCodes}
import dataparsers.util.Language
import util.text.ParseExceptionIgnorer
import util.text.uri.UriDecoder
...
)
{
    ...
    val encoded = if (capitalizeLink) {
        WikiUtil.wikiEncode(decoded).capitalize
    }
    else {
    ...
    private def withNamespace(encode : Boolean) : String =
    {
      ...
      if (encode) ns = WikiUtil.wikiEncode(ns).capitalize
...
object WikiTitle
{
    ...
    def parse(title : String, sourceLanguage : Language): WikiTitle =
    {
        ...
        val decodedName = parts.mkString(":").trim.capitalize
