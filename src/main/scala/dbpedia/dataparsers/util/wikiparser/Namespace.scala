...
import dbpedia.dataparsers.util.{Language, Namespaces}
...
private class NamespaceBuilder {
  ...
  // map from namespace name to namespace (only  namespaces)
...
private[wikiparser] class NamespaceBuilderDisposer(builder: NamespaceBuilder) {
  ...
  /**
   * Immutable map from language to namespace containing only the mapping namespaces on http://mappings..org.
   */
...
object Namespace extends NamespaceBuilderDisposer(new NamespaceBuilder) {
  ...
  def getValues(): Map[Int, Namespace] = {
    values
  }
